...
void
randomList(List<T>& list, int n) {  //在[0, 2n)中选择n个偶数，随机插入列表
  ListNodePosi(T) p = (rand() % 2) ? list.insertAsBack(rand() % n * 2)
                                   : list.insertAsFront(rand() % n * 2);
  for(int i = 1; i < n; i++)
    p = rand() % 2 ? list.insert(p, rand() % n * 2, InsertType::INSERT_BEFORE)
                   : list.insert(p, rand() % n * 2, InsertType::INSERT_AFTER);
}
...
testList(int testSize) {
  printf(
    ...
    testID++, testSize);
  List<T> La;
  randomList(La, testSize);
  PRINT(La);
  List<T> Lb;
  randomList(Lb, testSize);
  PRINT(Lb);
  ...
  PRINT(Lb);
  while(0 < Lb.size())
    La.insertAsBack(Lb.remove(Lb.begin()));
  PRINT(La);
  PRINT(Lb);
