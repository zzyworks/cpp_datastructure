...
template<typename T>  //元素类型
void
TestOrderedInsertion(Vector<T>& V, int n) {
  while(n * 2 > V.size()) {
    T e = dice((T)n * 2);
    printf("Inserting ");
    print(e);
    printf(" ...\n");
    V.insert(V.begin() + V.search(e) + 1, e);
    print(V);
  }
}
...
template<typename T>  //元素类型
void
testVector(int testSize) {
  printf("\n  ==== Test %2d. Generate a random vector\n", testID++);
  Vector<T> V;
  for(int i = 0; i < testSize; i++)
    V.insert(V.begin() + dice(i + 1),
             dice((T)testSize * 3));  //在[0, 3n)中选择n个数，随机插入向量
  PRINT(V);
  permute(V);
  PRINT(V)
  ...
  PRINT(V);
  for(int i = 0; i < V.size(); i += V.size() / 5) {
    V.sort(0, i, std::min(V.size(), i + V.size() / 5));
    PRINT(V);
